...
void Tree::Assign_Code(Node* tree, int huffmCode[], int n)
{
    ...
    if ((tree->left == NULL) && (tree->right == NULL))
    {
        ...
        Tree::map_char_freq_as_int.insert(pair);
...
void Tree::update_to_strings_only(){
    
    for (std::map<char*,vector<int> >::iterator it = Tree::map_char_freq_as_int.begin();it!=Tree::map_char_freq_as_int.end();++it){
        ...
        // store freq as a string
        string freq_as_string = ss.str();
        
        std::pair<char*,string > pair(it->first,freq_as_string);
        Tree::map_char_freq_as_string.insert(pair);
        cout << endl;
    }
}

void Tree::printMappings(){
    for (std::map<char*,string >::iterator it = Tree::map_char_freq_as_string.begin();it!=Tree::map_char_freq_as_string.end();++it){
        cout << it->first << "->";
        for (int i = 0; i < it->second.size(); i++) {
            cout << it->second.at(i);
        }
